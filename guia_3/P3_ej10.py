...
def graficar_transformacion_esfera(epsilons):
    ...
    X = np.array([np.cos(theta), np.sin(theta)]) # Es la circunf unidad
    # graficamos circunf unid
    ...
    for eps in epsilons:
        A_eps = np.array([[1.0, 1.0 - eps], [0.0, 1.0]]) # A de epsilon en el inciso (a)

        # Transformación de los puntos
        Y = A_eps @ X # modifica la circunf unidad multiplicandola por el A de epsilon(Matriz)
...
#Esta función dibuja una línea horizontal que cruza todo el gráfico (el eje X de referencia o en el ejr Y).
#Estas funciones sirven para escribir el título o etiqueta que aparece abajo (eje X) y al costado (eje Y) del cuadro del gráfico.
